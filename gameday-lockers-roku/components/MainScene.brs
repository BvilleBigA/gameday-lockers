sub init()
  ...
  if m.fxTimer <> invalid then m.fxTimer.control = "start"
end sub

sub onPollTimer()
    print "Poll Timer Fired: Checking server for pairing status..."
    
    ' ... the rest of your server checking code ...
    end sub
  ...
  if m.currentMode = "pairing"
    ...
    if m.lastMediaUrl <> "" and m.contentPoster <> invalid then
      m.contentPoster.visible = true
      m.contentPoster.opacity = 1.0
      m.contentPoster.uri = m.lastMediaUrl
    end if
...
sub handleCurrentSceneResult(result as Object)
  ...
  if scene.backgroundUrl <> invalid and scene.backgroundUrl <> ""
    ...
    print "Setting Poster URI to: "; mediaUrl
    m.lastMediaUrl = mediaUrl
    m.contentPoster.visible = true
    m.contentPoster.opacity = 1.0
    m.contentPoster.uri = mediaUrl
  end if
  showContentGroup()
end sub
...
sub showContentGroup()
  ...
  if m.contentPoster <> invalid then
    m.contentPoster.visible = true
    m.contentPoster.opacity = 1.0
  end if
  hideOverlayUi()
